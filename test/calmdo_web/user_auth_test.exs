defmodule CalmdoWeb.UserAuthTest do
  use CalmdoWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Calmdo.Accounts
  alias Calmdo.Accounts.Scope
  alias CalmdoWeb.UserAuth

  import Calmdo.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, CalmdoWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      CalmdoWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        CalmdoWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
